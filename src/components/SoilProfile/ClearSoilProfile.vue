<template>
  <div>
    ...
    <br>
    <h3>Are you sure do you want to reset all the records of this soil profile? This cannot be undone.</h3>

    <button id="trig-btn" class="normal-btn normal-btn soil-btn exit-btn" v-on:click="clear">Clear</button>
...
export default {
  ...
  methods: {
    clear() {
      axios.post("/clear_soil_profile", {
        'soil_profile_id' : this.soil_profile_id
      })
            .then((response) => {
              this.$notify({
                  group: 'notify',
                  title: 'Reset Soil Profile',
                  text: response.data
                })
              router.push('main')
            })
